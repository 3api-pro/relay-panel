...
任意装了 Docker 的机器都可以跑 — Linux / macOS / Windows (Docker Desktop)。
1 GB 内存起步:

```bash
git clone https://github.com/3api-pro/relay-panel
cd relay-panel
cp .env.example .env          # 改 POSTGRES_PASSWORD / JWT_SECRET / UPSTREAM_KEY
docker compose up -d
```

打开 `http://localhost:8080` → 注册第一个 admin → 走完引导向导 (上游配置 / 品牌 /
首套餐) → 完成。要挂公网域名走自己的反向代理, 或者用仓库里的 `Caddyfile`
自动签 TLS。

不想运维? 直接去 [3api.pro/create](https://3api.pro/create), 同一份面板我们托管, 5 秒开店。
