...
export default function FinancePage() {
  ...
  const topupCols: Column<TopupRow>[] = [
    ...
    { key: 'ref', header: '凭证', render: (r) => <span className="text-xs font-mono text-muted-foreground">{r.ref ?? '—'}</span> },
    { key: 'status', header: '状态', render: (r) => <span className="text-xs">{r.status ?? '—'}</span> },
    { key: 'time', header: '时间', render: (r) => <span className="text-xs text-muted-foreground">{fmtDate(r.created_at)}</span> },
  ];

  const planCols: Column<PlanRevenue>[] = [
    { key: 'plan', header: '套餐', render: (p) => <span className="text-foreground">{p.plan_name}</span> },
    ...
    <AdminShell title="财务" subtitle="上游充值 + 收入报表 + 提现">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <section className="lg:col-span-2 bg-card rounded-lg border border-border p-6">
          <div className="text-xs text-muted-foreground uppercase tracking-wide">Wholesale 余额</div>
          <div className={
          ...
          <div className="text-xs text-muted-foreground mt-1">更新于 {fmtDate(ws?.updated_at ?? null)}</div>
          {lowBalance && (
        ...
        <section className="bg-card rounded-lg border border-border p-6">
          <div className="text-xs text-muted-foreground uppercase tracking-wide">本月毛利估算</div>
          <div className="text-2xl font-bold text-foreground mt-2">
            {fmtCNY(planRev.reduce((acc, p) => acc + (p.revenue_cents ?? 0), 0))}
          </div>
          <div className="text-xs text-muted-foreground mt-1">30 天滚动 · 不含未结算</div>
      ...
      <section className="mb-6">
        <h2 className="font-semibold text-foreground mb-3">充值历史</h2>
        <DataTable
          rows={ws?.history ?? []}
          columns={topupCols}
          keyFn={(r) => r.id}
      ...
      <section>
        <h2 className="font-semibold text-foreground mb-3">30 天收入（按套餐）</h2>
        <DataTable
          rows={planRev}
          columns={planCols}
          keyFn={(p) => p.plan_name}
      ...
      >
        <div className="space-y-3 text-sm">
          <div>
            <div className="text-xs font-medium text-muted-foreground mb-1">金额（分）</div>
            <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)}
              className="w-full px-3 py-2 rounded-md border border-input" />
            <div className="text-xs text-muted-foreground mt-0.5">= {fmtCNY(Number(amount) || 0)}</div>
          </div>
          <div>
            <div className="text-xs font-medium text-muted-foreground mb-1">支付方式</div>
