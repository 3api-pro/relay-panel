...
export default function DocsPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12 space-y-6">
      <h1 className="text-3xl font-bold tracking-tight text-foreground">API 文档</h1>
      <p className="text-muted-foreground">本站 API 与 Anthropic Messages API 完全兼容, 任何 Anthropic 兼容的客户端都可直接接入。</p>
      ...
      <Card title="认证">
        <p className="text-sm text-foreground mb-3">
          请在 <Link href="/dashboard/keys" className="underline" style={{ color: 'var(--brand-primary, #0e9486)' }}>API Keys</Link> 页生成 <code className="text-xs bg-muted px-1 rounded">sk-*</code> 密钥, 然后通过 <code className="text-xs bg-muted px-1 rounded">Authorization: Bearer</code> 头传入。
        </p>
        <Alert kind="info">
          也支持 <code className="text-xs bg-card px-1 rounded">x-api-key</code> 头, 与 Anthropic 官方完全兼容。
        </Alert>
      </Card>
    ...
    "messages": [
      ...
      <Card title="官方 SDK">
        <div className="text-sm space-y-3">
          <div>
            <div className="text-muted-foreground mb-1">Python (anthropic):</div>
...
print(msg.content[0].text)`}</pre>
          ...
          <div>
            <div className="text-muted-foreground mb-1">TypeScript (@anthropic-ai/sdk):</div>
...
console.log(msg.content[0].text);`}</pre>
      ...
      <Card title="客户端配置">
        <div className="text-sm text-foreground space-y-2">
          <p><strong>Claude Code / Cursor / Cline:</strong> 把 <code className="text-xs bg-muted px-1 rounded">ANTHROPIC_BASE_URL</code> 设为 <code className="text-xs bg-muted px-1 rounded">{origin}</code>, <code className="text-xs bg-muted px-1 rounded">ANTHROPIC_API_KEY</code> 设为你的 <code className="text-xs bg-muted px-1 rounded">sk-*</code>。</p>
