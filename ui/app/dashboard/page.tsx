      ...
      <header className="bg-white border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-sm">
            ...
            <button onClick={logout} className="text-slate-600 hover:text-red-600">退出</button>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        ...
        <section className="bg-white rounded-lg border border-slate-200 p-6">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {tokens.length === 0 ? (
                <tr><td colSpan={5} className="py-4 text-center text-slate-500">还没有 API Key</td></tr>
              ) : tokens.map(t => (
                <tr key={t.id} className="border-b border-slate-100">
                  <td className="py-3">{t.name}</td>
                  <td><code className="text-xs">{t.key_prefix}…</code></td>
                  <td><span className={`text-xs px-2 py-0.5 rounded ${t.status==='active' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>{t.status}</span></td>
        ...
        <section className="bg-white rounded-lg border border-slate-200 p-6">
          ...
          <div className="flex gap-2">
            <input type="text" value={code} onChange={(e)=>setCode(e.target.value)}
              placeholder="输入兑换码"
              className="flex-1 px-3 py-2 rounded-md border border-slate-300 focus:border-brand-500 focus:outline-none" />
